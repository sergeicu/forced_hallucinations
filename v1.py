...
def calculate_accuracy_score(extracted_date: str, ground_truth_date: str) -> Tuple[bool, float]:
    ...
    try:
        ...
        if full_date_correct:
            ...
            month_diff = abs(extracted.month - ground_truth.month)
            day_diff = abs(extracted.day - ground_truth.day)
            confidence = max(0.5, 1.0 - (month_diff + day_diff) / 24.0)
        else:
            # Year is wrong
            year_diff = abs(extracted.year - ground_truth.year)
            confidence = max(0.0, 1.0 - year_diff / 100.0)
